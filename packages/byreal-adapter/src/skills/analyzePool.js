import { PoolAnalysisInputSchema, PoolAnalysisResultSchema, } from "../types.js";
 ...
 */
export function analyzePoolMock(rawInput, options) {
    ...
    const utilizationPct = round(5 + rng() * 120);
    const volume24hUsd = round((tvlUsd * utilizationPct) / 100);
    const feeBps = [5, 30, 100][Math.floor(rng() * 3)] ?? 30;
    const estimatedAprPct = round(((volume24hUsd * (feeBps / 10_000) * 365) / Math.max(tvlUsd, 1)) * 100);
    ...
    const pairLabel = input.pairLabel ?? `POOL-${input.poolAddress.slice(2, 8).toUpperCase()}`;
    const signals = [];
    signals.push(tvlUsd >= 1_000_000
    ...
    signals.push(utilizationPct > 90
