import { PositionInputSchema, PositionResultSchema, } from "../types.js";
 ...
 */
export function managePositionMock(rawInput, options) {
    ...
    const riskScore = clampScore((status === "closed" ? 0 : 25) +
    ...
    if (status === "closed") {
        ...
        recommendations.push("Monitor price band; rebalance if the pair drifts outside your range.");
        if (riskScore > 50) {
            recommendations.push("Elevated risk — consider reducing size or tightening the range.");
        }
    ...
    const body = {
        ...
        riskScore,
        recommendations,
        humanSummary: `Position ${positionId} after "${input.action}": status ${status}, ` +
