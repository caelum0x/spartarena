 ...
 */
export function previewSwapMock(
  rawInput: SwapPreviewInput,
  ...
  const riskScore = clampScore(
    priceImpactPct * 18 + (input.slippageBps > 200 ? 20 : 0) + rng() * 10,
  );
  ...
  const body = {
    ...
    riskScore,
    humanSummary:
      `Swapping ${input.amountIn} ${input.tokenIn} -> ~` +
  ...
  const result: SwapPreviewResult = {
    ...body,
