...
const TOKEN_UNIVERSE: ReadonlyArray<{ symbol: string; name: string }> = [
...
function seedFromHex(seed: string): string {
 ...
 */
export function discoverTokenMock(
  ...
  const tokens: DiscoveredToken[] = TOKEN_UNIVERSE.slice()
    .map((token) => {
    ...
    .sort(
      (a, b) =>
        b.liquidityScore - b.riskScore - (a.liquidityScore - a.riskScore),
    )
    .slice(0, limit);
  ...
  const body = {
    ...
    confidence,
    humanSummary:
      `Discovered ${tokens.length} candidate token(s) for "${input.query}" on ` +
