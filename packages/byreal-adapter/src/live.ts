...
import {
  ...
  SwapPreviewInputSchema,
  SwapPreviewResultSchema,
  TokenDiscoveryInputSchema,
  TokenDiscoveryResultSchema,
  type ByrealChain,
 ...
 */
export function analyzePoolInfo(
  pool: SimplePoolInfo,
  options: AnalyzePoolInfoOptions = {},
): PoolAnalysisResult {
  const chain: ByrealChain = options.chain ?? "mantle-sepolia";
...
/** Options controlling how a batch of {@link MintInfo} rows is ranked. */
export interface BuildTokenDiscoveryOptions {
  /** Settlement/data chain label recorded in the discovery body. */
  chain?: ByrealChain;
  /** The query/intent label this discovery answers (e.g. "trending"). */
  query: string;
  /** Whether the resulting proof hash is flagged as recorded on Mantle. */
  recordedOnMantle?: boolean;
}
...
/**
 * Pure, deterministic ranking of already-listed Byreal token rows into a
 * proof-backed {@link TokenDiscoveryResult}. Extracted from
 * {@link LiveByrealAdapter.discoverToken} so a token-board route can rank a
 * pre-fetched mint list without re-querying. Same input → same hash.
 */
export function buildTokenDiscovery(
  mints: readonly MintInfo[],
  options: BuildTokenDiscoveryOptions,
): TokenDiscoveryResult {
  const chain: ByrealChain = options.chain ?? "mantle-sepolia";

  const tokens: DiscoveredToken[] = mints.map((m) => {
    ...
    const riskScore = clampScore(
      80 - liquidityScore * 0.5 + Math.abs(priceChange24hPct) * 0.6,
    );
    const reason =
  ...
  const body = {
    ...
    confidence,
    humanSummary:
      `Discovered ${tokens.length} Byreal token(s) for "${options.query}" ` +
 ...
 */
export class LiveByrealAdapter implements ByrealSkillAdapter {
  private readonly client: ByrealRestClient;
  ...
  async analyzePool(rawInput: PoolAnalysisInput): Promise<PoolAnalysisResult> {
    const input = PoolAnalysisInputSchema.parse(rawInput);

    let pool: SimplePoolInfo | null = await this.client.getPoolDetails(
      input.poolAddress,
    );
    if (!pool) {
    ...
    if (!pool) {
      throw new Error(
        `Byreal pool not found for address ${input.poolAddress}`,
      );
    }
  ...
  async discoverToken(
    ...
    return buildTokenDiscovery(top, {
      chain: input.chain,
      query: input.query,
      recordedOnMantle: this.recordedOnMantle,
    });
  ...
  async previewSwap(rawInput: SwapPreviewInput): Promise<SwapPreviewResult> {
    ...
    const mints = await this.client
      ...
      .catch(() => []);
    const inMint =
      mints.find((m) => m.address === input.tokenIn) ??
    ...
    const outMintList = await this.client
      ...
      .catch(() => []);
    const outMint =
      outMintList.find((m) => m.address === input.tokenOut) ??
    ...
    const quote: SwapQuote = await this.client.getSwapQuote({
    ...
    const expectedAmountOut = fromBaseUnits(
      quote.outAmount ?? "0",
      outDecimals,
    );
    const minAmountOut = fromBaseUnits(
      quote.otherAmountThreshold ?? quote.outAmount ?? "0",
      outDecimals,
    );
    const amountInNum = Number.parseFloat(input.amountIn);
    const expectedOutNum = Number.parseFloat(expectedAmountOut);
    const executionPrice =
      amountInNum > 0 ? round(expectedOutNum / amountInNum, 8) : 0;
    ...
    const route =
      quote.poolAddresses && quote.poolAddresses.length > 0
        ? [input.tokenIn, ...quote.poolAddresses, input.tokenOut]
        : [input.tokenIn, input.tokenOut];

    const riskScore = clampScore(
      priceImpactPct * 18 + (input.slippageBps > 200 ? 20 : 0),
    );
    ...
    const body = {
      ...
      riskScore,
      humanSummary:
        `Byreal (Solana) preview: ${input.amountIn} ${input.tokenIn} -> ~` +
   ...
   */
  async managePosition(rawInput: PositionInput): Promise<PositionResult> {
    ...
    const positions = owner
      ...
      : [];
    const match =
      positions.find((p) => p.poolAddress === input.poolAddress) ??
      positions[0];
    ...
    const status: PositionResult["status"] =
      Number.parseFloat(liquidity) > 0 ? "open" : "closed";
    ...
    if (status === "open") {
      recommendations.push(
        "Monitor the price band; rebalance on Solana if the pair drifts out of range.",
      );
    }
    ...
    const body = {
      ...
      recommendations,
      humanSummary:
        `Byreal (Solana) position read for pool ${input.poolAddress}: ` +
